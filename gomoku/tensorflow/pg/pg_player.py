'''
강화학습 Policy Gradient를 사용한 오목 AI
'''
...
def playGame(env, memory, sess, saver, epsilon, iteration):
	...
	for i in range(epoch):
		...
		while(gameOver != True):
			...
			inputs, targets = memory.getBatch(
				output_layer, batchSize, nbActions, nbStates, sess, X)
	...
	print(("Epoch " + str((iteration + 1) * epoch) + ", Error = " +
            str(err) + ", Win ratio = " + str(float(winCount) / float(epoch) * 100)))
#------------------------------------------------------------
